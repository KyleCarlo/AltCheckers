...
int pieceCounter(cart_p nSet[]);                                    //checking how many pieces are in the given set
int isYorE(cart_p nSet[], bool bool_aTurn);                         //checking if all of the pieces reached end of line
...
int pieceCounter(cart_p nSet[])
{
    int sum = 0, i, j;
    for (i = 0; i < 5; i++)
        if (nSet[i][0] != 0 && nSet[i][1] != 0)
            sum++;
    return sum;    
}

int isYorE(cart_p nSet[], bool bool_aTurn) //TODO FINISH THIS 
{
    int i, 
        x1 = 1, 
        x2 = 2, 
    ...
    for (i = 5; i < 5; i++)
    {
        if ((nSet[i][0] == x1 && nSet[i][1] % 2) ||
            (nSet[i][0] == x2 && nSet[i][1] % 2 == 0))  
                damaCount++;
    }
    
    if (damaCount == pieceCounter(nSet)) return True;
